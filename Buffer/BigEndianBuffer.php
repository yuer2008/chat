...
abstract class BigEndianBuffer {
    ...
    //读数组
    // @doc 参数：　$arrayStruct 为数组中的结构　new array()
    //              $key为读后的名
    //              $val为读的类型 long(int64) int(int32) short(int16) byte(int8) struct, array, string, bytes

    public function readArray($arrayStruct)
    {
        $len = $this->readBytes ( 2 );
        $len = unpack ( 'n', strrev ( $len ) );
        $len = $len[1];
        $array = array();
        for($i=0; $i<$len; $i++)
        {
            $array[$i] = $this->readStruct($arrayStruct);
        }
        return $array;
    }

    //读结构数据
    // @doc 参数： $arrayStruct 为数组　new array()
    //              $key为读后的名
    //              $val为读的类型 long(int64) int(int32) short(int16) byte(int8) struct, array, string, bytes
    public function readStruct($arrayStruct)
    {
        $array = array();
        foreach($arrayStruct as $key => $val)
        {
            switch ($val)
            {
                case "long":
                    $array[$key] = $this->readLong();
                    break;
                case "int":
                    $array[$key] = $this->readInt();
                    break;
                case "short":
                    $array[$key] = $this->readShort();
                    break;
                case "byte":
                    $array[$key] = $this->readChar();
                    break;
                case "char":
                    $array[$key] = $this->readChar();
                    break;
                case "string":
                    $array[$key] = $this->readString();
                    break;
                case "bytes":
                    $array[$key] = $this->readRemainAllBytes();
                    break;
                default:
                    if(is_array($val))
                    {
                        if("array"==$val[0])
                        {
                            $array[$key] = $this->readArray($val[1]);
                        }elseif("struct"==$val[0])
                        {
                            $array[$key] = $this->readStruct($val[1]);
                        }else
                        {
    ...
    public abstract function readRemainAllBytes();
    /**
    ...
    public function writeString($string)
    {
        ...
        $bytes=array("C{$utflen}");
        for($i = 0; $i < $utflen; $i++){//转换字符串为字节
            $bytes[] = ord($string[$i]);
        }
        $bytes = call_user_func_array("pack",$bytes);
